...
<div>
    ...
    @if ($stats != [])
        <div class="gray-card w-fit mt-6">
            ...
            <div>
                <p>{{ $habTypeName }}生育地類型，共記錄 {{ $stats['total_species'] }} 種植物，分屬 {{ $stats['total_families'] }}
                    科、{{ $stats['total_genera'] }} 屬。</p>
                <p>其中歸化植物 {{ $stats['naturalized_species'] }} 種，占全部物種之 {{ $stats['naturalized_ratio'] }}%；原生植物
                    {{ $stats['native_species'] }} 種，包含特有種 {{ $stats['endemic_species'] }} 種。</p>

                <p>植物生長習性以{{ $stats['growth_form'][0]['growth_form'] }}為最多數，占全部物種之
                    {{ round($stats['growth_form'][0]['growth_form_count'] / $stats['total_species'], 1) }}%；其次為{{ $stats['growth_form'][1]['growth_form'] }}，約佔
                    {{ round($stats['growth_form'][1]['growth_form_count'] / $stats['total_species'], 1) }}%；以{{ $stats['growth_form'][count($stats['growth_form']) - 1]['growth_form'] }}為最少，約占
                    {{ round($stats['growth_form'][count($stats['growth_form']) - 1]['growth_form_count'] / $stats['total_species'], 1) }}%。
                </p>
                @if ($stats['naturalized_species'] > 0)
                    <p>歸化植物共{{ $stats['naturalized_species'] }}種，分屬{{ $stats['naturalized_families'] }}科，{{ $stats['naturalized_genera'] }}個屬。以{{ $stats['naturalized_growth_form'][0]['growth_form'] }}最常見，佔全部歸化植物之
                        {{ round($stats['naturalized_growth_form'][0]['growth_form_count'] / $stats['total_species'], 1) }}%。
                    </p>
                @endif
            </div>
